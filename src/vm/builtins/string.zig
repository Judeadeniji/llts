...
fn srcOf(vm: *VMState, v: Value, buf: *std.ArrayList(u8)) !ArenaSrc {
    return switch (v) {
        ...
        .bytes => |b| .{ .off = null, .interned = vm.bytes.items[b.offset..][0..b.len], .len = b.len },
        .ptr => blk: {
...
pub fn register(vm: *VMState) !void {
    ...
    try vm.defineGlobal("__strlen", .{ .native = &strlen_n });
    try vm.defineGlobal("__substr", .{ .native = &substr_n });
    try vm.defineGlobal("__indexOf", .{ .native = &index_of_n });
    try vm.defineGlobal("__split", .{ .native = &split_n });
    try vm.defineGlobal("__toUpper", .{ .native = &to_upper_n });
    try vm.defineGlobal("__toLower", .{ .native = &to_lower_n });
    try vm.defineGlobal("__trim", .{ .native = &trim_n });
    try vm.defineGlobal("__replace", .{ .native = &replace_n });
    try vm.defineGlobal("__concat", .{ .native = &concat_n });
    try vm.defineGlobal("__repeat", .{ .native = &repeat_n });
    try vm.defineGlobal("__startsWith", .{ .native = &starts_with_n });
    try vm.defineGlobal("__endsWith", .{ .native = &ends_with_n });
    try vm.defineGlobal("__charCodeAt", .{ .native = &char_code_at_n });
    try vm.defineGlobal("__parseInt", .{ .native = &parse_int_n });
    try vm.defineGlobal("__parseFloat", .{ .native = &parse_float_n });
    try vm.defineGlobal("__fromCharCode", .{ .native = &from_char_code_n });
    ...
    try vm.defineGlobal("__contains", .{ .native = &contains_n });
    try vm.defineGlobal("__lastIndexOf", .{ .native = &last_index_of_n });
    try vm.defineGlobal("__indexOfFrom", .{ .native = &index_of_from_n });
    try vm.defineGlobal("__trimStart", .{ .native = &trim_start_n });
    try vm.defineGlobal("__trimEnd", .{ .native = &trim_end_n });
    try vm.defineGlobal("__replaceFirst", .{ .native = &replace_first_n });
    try vm.defineGlobal("__slice", .{ .native = &slice_n });
    try vm.defineGlobal("__compare", .{ .native = &compare_n });
    try vm.defineGlobal("__eql", .{ .native = &eql_n });
    try vm.defineGlobal("__splitMax", .{ .native = &split_max_n });
    try vm.defineGlobal("__join", .{ .native = &join_n });
    try vm.defineGlobal("__padStart", .{ .native = &pad_start_n });
    try vm.defineGlobal("__padEnd", .{ .native = &pad_end_n });
    try vm.defineGlobal("__isEmpty", .{ .native = &is_empty_n });
    try vm.defineGlobal("__isBlank", .{ .native = &is_blank_n });
}
