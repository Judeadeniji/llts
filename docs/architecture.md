...
- **Explicit Memory Management**: LLTS eschews tracing Garbage Collection (GC) in favor of explicit ownership. It introduces a frame-local heap (where bare initializations like `Foo{}` or `[...]` are bump-allocated and automatically rewound when the frame returns). Returning a frame-local *variable* is a compile error; returning a bare literal `Foo{…}` / `[…]` immortalizes it. Reclaimable heap uses `@new` with `std.mem.Arena` and `defer` for cleanup.
